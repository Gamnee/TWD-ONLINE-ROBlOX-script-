...
local TeleportLocations = {
    -- 🟡 Yellow locations first
    {Name = "Airport", CFrame = CFrame.new(3338.46, 143.08, 3531.80), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "FarmLand", CFrame = CFrame.new(5158.74, 237.88, 3581.21), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "Farmhouse", CFrame = CFrame.new(3434.85, 141.66, 5581.14), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "Hospital", CFrame = CFrame.new(-2323.73, 174.51, 5059.67), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "King County", CFrame = CFrame.new(-3235.45, 188.79, 4728.39), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "Hilltop", CFrame = CFrame.new(-499798, 223.28, -441.94), Color = Color3.fromRGB(255, 215, 0)},
    {Name = "Sanctuary", CFrame = CFrame.new(-5250.66, 167.27, -3621.97), Color = Color3.fromRGB(255, 215, 0)},
    ...
    {Name = "Prison", CFrame = CFrame.new(5410.80664, 133.174713, -3000.37769, 0.726467907, -4.40537313e-08, 0.687200427, -1.15541052e-08, 1, 7.63204113e-08, -0.687200427, -6.33847629e-08, 0.726467907), Color = Color3.fromRGB(255, 215, 0)},
    ...
    -- ⚪ White locations after yellow locations
...
-- ⭐ ปิดการใช้ TeamColor เพื่อไม่ให้สีฟ้ามาทับสี ESP 
...
local function saveOriginalValues(humanoid) 
    ...
    if success 
        and typeof(useJumpPower) == "boolean" 
        and typeof(jumpPower) == "number" then 
 
        originalUseJumpPower = useJumpPower 
        originalJumpPower = 
            jumpPower > 0 and jumpPower or 50 
...
local function updateFOVCircle() 
    ...
    FOVStroke.Enabled = 
        Settings.FOVThickness > 0 
 
    FOVFrame.Visible = 
        Features.TargetLock.Enabled 
end 
...
-- 
-- จุดสำคัญ: 
-- ไม่เอา TeamColor มาทับสี ESP 
-- สีที่เลือกใน ESPColor จะถูกใช้ทั้ง 
-- Highlight + NameTag 
-------------------------------------------------------------------- 
...
local function updateESP() 
    ...
    for _, player in ipairs(Players:GetPlayers()) do 
        ...
        if character then 
            ...
            local highlight = 
                character:FindFirstChild("GaneInwzaHighlight") 
            ...
            local nameTag = 
                holder:FindFirstChild("PlayerNameTag") 
            ...
            if Features.ESP.Enabled and head then 
                ...
                -- ⭐ แสดงชื่อของตัวเองด้วย 
                nameTag.Tag.Text = 
                    player.Name 
 
                -- ⭐ ชื่อใช้สีเดียวกับ Highlight 
                nameTag.Tag.TextColor3 = 
                    color 
...
VehicleHolder.Parent = 
    game.CoreGui 
 
local VehicleTagTemplate = 
    Instance.new("BillboardGui") 
 
VehicleTagTemplate.Name = 
    "VehicleTag" 
 
VehicleTagTemplate.Size = 
    UDim2.new(0, 200, 0, 50) 
 
VehicleTagTemplate.AlwaysOnTop = true 
 
VehicleTagTemplate.StudsOffset = 
    Vector3.new(0, 2, 0) 
...
local function trackSeat(seat) 
    ...
    if seat:IsA("VehicleSeat") 
        and not table.find(cachedSeats, seat) then 
...
local function updateVehicleESP() 
    ...
    for i = #cachedSeats, 1, -1 do 
        ...
        if not seat 
            ...
            if not Players:GetPlayerFromCharacter( 
                vehicleModel 
            ) then 
...
local function updateStatusDisplay()
    ...
    StatusGui.Visible = activeCount > 0
end
...
ChatHead.MouseButton1Click:Connect( 
    function() 
...
TeleportContainer.Parent = 
    MainFrame 
 
local TeleportUIList = 
    Instance.new("UIListLayout") 
...
local function createFeatureRow( 
    ...
    SwitchBG.MouseButton1Click:Connect( 
        function() 
...
--------------------------------------------------------------------
-- 📌 Dynamic Teleport: Vehicles / Players
--------------------------------------------------------------------

local DynamicTeleportPrefix = "__DynamicTeleport_"

local function createTeleportSectionLabel(name, text, color)
    local label = Instance.new("TextLabel")
    label.Name = DynamicTeleportPrefix .. name
    ...
    label.Font = Enum.Font.GothamBold
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Parent = TeleportContainer
end

local function createDynamicTeleportButton(name, text, color, callback)
    local button = Instance.new("TextButton")
    button.Name = DynamicTeleportPrefix .. name
...
local function clearDynamicTeleportRows()
    for _, child in ipairs(TeleportContainer:GetChildren()) do
        if child.Name:sub(1, #DynamicTeleportPrefix) == DynamicTeleportPrefix then
            child:Destroy()
        end
    end
end

local function getDistanceToPosition(position)
...
local function findVehiclePart(model)
    if not model then
        return nil
    end
...
local function collectVehicles()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("VehicleSeat") then
            ...
            if model and not seen[model] then
                local part = findVehiclePart(model)
    ...
    table.sort(vehicles, function(a, b)
        ...
        if an == bn then
            return getDistanceToPosition(a.Part.Position)
                < getDistanceToPosition(b.Part.Position)
        end
...
local function collectPlayers()
    ...
    table.sort(players, function(a, b)
        return string.lower(a.Player.Name)
            < string.lower(b.Player.Name)
    end)
...
local function updateDynamicTeleportUI()
    clearDynamicTeleportRows()

    createTeleportSectionLabel(
        "VehicleHeader",
        "🚗 " .. LangText[CurrentLanguage].VehicleList,
    ...
    if #vehicles == 0 then
        createDynamicTeleportButton(
            "NoVehicles",
            LangText[CurrentLanguage].NoVehicles,
            ...
            local distance =
                math.floor(
                    getDistanceToPosition(
                        vehicle.Part.Position
                    )
                )

            createDynamicTeleportButton(
                "Vehicle_" .. tostring(index),
                "🚗 "
                    .. vehicle.Name
                    .. " ["
                    .. tostring(distance)
    ...
    createTeleportSectionLabel(
        "PlayerHeader",
        "👤 " .. LangText[CurrentLanguage].PlayerList,
    ...
    if #players == 0 then
        createDynamicTeleportButton(
            "NoPlayers",
            LangText[CurrentLanguage].NoPlayers,
            Color3.fromRGB(150, 150, 160),
            function() end
        )
    else
...
StatusBtn.Position = UDim2.new(1, -120, 0.5, -13)
StatusBtn.BackgroundColor3 = Color3.fromRGB(38, 38, 50)
...
MenuKeyBtn.MouseButton1Click:Connect( 
    function() 
...
ESPColorBtn.MouseButton1Click:Connect( 
    function() 
...
LangBtn.MouseButton1Click:Connect( 
    function() 
...
ThemeBtn.MouseButton1Click:Connect( 
    function() 
...
TargetBtn.MouseButton1Click:Connect( 
    function() 
 
        if Settings.TargetPart 
            == "Head" then 
